 ...
 * @file
 *
 * Defines the {@link SpectrogramShape} class.
 *
 ...
 *
 * @typedef {Object} SpectrogramShapeOptions
 * @global
 ...
 *
 * @class
 * @alias SpectrogramShape
 *
 * @param {SpectrogramShapeOptions} options
 */

function SpectrogramShape(options) {
...
// SpectrogramShape.prototype = Object.create(Konva.Shape.prototype);

SpectrogramShape.prototype.setSegment = function(segment) {
  this._segment = segment;
};

SpectrogramShape.prototype.setWaveformColor = function(color) {
  if (isString(color)) {
...
SpectrogramShape.prototype.fitToView = function() {
  this.setWaveformColor(this._color);
};

SpectrogramShape.prototype._sceneFunc = function(context) {
...
SpectrogramShape.prototype._drawWaveform = function(context, waveformData,
...
SpectrogramShape.prototype._drawChannel = function(context, channel,
  ...
  for (x = startPixels; x <= endPixels; x++) {
    ...
    lineY = top + SpectrogramShape.scaleY(amplitude, height, amplitudeScale) + 0.5;
...
SpectrogramShape.prototype._waveformShapeHitFunc = function(context) {
  if (!this._segment) {
    return;
  }
...
SpectrogramShape.prototype.addToLayer = function(layer) {
  layer.add(this._shape);
};

SpectrogramShape.prototype.destroy = function() {
  this._shape.destroy();
  this._shape = null;
};

SpectrogramShape.prototype.on = function(event, handler) {
...
SpectrogramShape.scaleY = function(amplitude, height, scale) {
  var y = -(height - 1) * (amplitude * scale + 128) / 255 + (height - 1);

  return clamp(Math.floor(y), 0, height - 1);
};

export default SpectrogramShape;
